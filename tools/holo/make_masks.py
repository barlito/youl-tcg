...
Usage:
    make_masks.py <artwork> [<artwork>...] [--out DIR] [--blur N] [--only subject|background]

Several artworks share one rembg session (the model load dominates the cost),
which is what makes per-extension batches usable.
"""
import argparse
import os
...
def main() -> None:
    ...
    parser.add_argument("artworks", nargs="+", help="path(s) to the source artwork(s)")
    ...
    for artwork in args.artworks:
        source = Image.open(artwork).convert("RGBA")
        cutout = remove(source, session=session)
        alpha = cutout.getchannel("A")
        if args.blur > 0:
            alpha = alpha.filter(ImageFilter.GaussianBlur(args.blur))

        out_dir = args.out or os.path.dirname(os.path.abspath(artwork))
        os.makedirs(out_dir, exist_ok=True)
        base = os.path.splitext(os.path.basename(artwork))[0]
