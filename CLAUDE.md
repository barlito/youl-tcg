...
- **Masquage des cartes inconnues** : dans le composeur, toute carte du joueur ciblé que le visiteur ne possède pas est rendue dos de carte (`parts/_card_back.html.twig`, sans contexte) sous le libellé « Carte inconnue » — elle reste demandable à l'aveugle (steppers et quantités actifs). La visibilité est calculée côté serveur (`getVisibleTheirCopies()` : les entrées masquées portent `card: null`, le nom ne peut donc pas fuiter dans le DOM), le filtre de recherche ne porte que sur les cartes visibles et n'ôte jamais une carte masquée (sinon il devient un oracle sur son nom). Même règle sur les lignes demandées des offres **envoyées**
