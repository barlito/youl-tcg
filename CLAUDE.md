...
**Échanges P2P (`TradeOffer` + `TradeOfferLine`, `src/Service/Trade/`):**
- Une offre = des exemplaires du proposeur (`OFFERED`) contre des exemplaires du destinataire (`REQUESTED`) ; statuts pending/accepted/refused/cancelled/invalidated
- **Réservation** : les copies engagées dans une offre PENDING par leur proposeur ne peuvent pas l'être dans une autre (registre recalculé depuis les lignes via `sumReservedQuantities()`, aucun compteur mutable). Le côté demandé n'est JAMAIS réservé — le destinataire n'a rien consenti ; il est re-vérifié à l'acceptation (et ses propres offres sortantes, elles, réservent)
- **Acceptation** : une transaction, `FOR UPDATE` sur les lignes `UserCard` des deux joueurs dans l'ordre global `(discordId, cardId)` (anti-deadlock), re-validation complète sous verrou, débit/crédit, puis transfert du `claimedBy` des uniques 1/1 par UPDATE conditionnel. `TradeOfferLine` stocke les quantités **séparées par finition** (normal/holo), contrairement à `UserCard` où `holoQuantity` est un sous-compte de `quantity`
- Le proposeur qui a perdu ses cartes → offre `INVALIDATED` (statut commité même si l'acceptation échoue) ; le destinataire qui ne peut pas couvrir → l'offre reste PENDING
- UI : `/echanges` (boîte + historique), `/echanges/nouveau[/{discordId}]` (composeur filtrable), badge d'offres en attente dans le header
- **Masquage des cartes inconnues (règle uniforme)** : partout dans l'écran d'échange — composeur, offres reçues, offres envoyées, quel que soit le côté offert/demandé — toute carte que le lecteur ne possède pas lui-même est rendue dos de carte (`parts/_card_back.html.twig`, inclus **sans contexte**) sous le libellé « Carte inconnue ». Elle reste demandable/acceptable à l'aveugle, et **sa rareté reste affichée** (pastille `--rarity-*` + libellé) : c'est à elle que se juge une offre. La visibilité est tranchée côté serveur — `TradeComposer::getVisibleTheirCopies()` et `TradeInbox::offeredView()/requestedView()` (DTO `TradeLineView`) portent `card: null` pour une entrée masquée, le nom ne peut donc pas fuiter dans le DOM ; le jeu d'ids possédés (`UserCardRepository::findOwnedCardIds()`) est mémoïsé, une requête par rendu. Le filtre de recherche ne porte que sur les cartes visibles et n'ôte jamais une carte masquée (sinon il devient un oracle sur son nom)
